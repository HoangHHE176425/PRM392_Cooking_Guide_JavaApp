...
import com.example.myapplication.connectDB.DatabaseConnection;
...
public class DishDAO {
    ...
    // Interface cho danh sách nguyên liệu
    public interface DishIngredientsCallback {
        void onSuccess(List<DishIngredient> ingredients);
        void onError(String error);
    }
        ...
        new AsyncTask<Void, Void, Object>() {
            ...
            protected Object doInBackground(Void... voids) {
                try {
                    ...
                    if (connection != null) {
                        // Fixed SQL query: Calculate total_calories from Dish_Ingredients
                        String query = "SELECT d.*, ISNULL(SUM(di.calories_per_unit), 0) as total_calories " +
                                "FROM Dishes d " +
                        ...
                        if (resultSet.next()) {
                            ...
                            dish.setTotalCalories(resultSet.getInt("total_calories"));
    ...
    // Lấy danh sách nguyên liệu của món ăn (bao gồm số lượng & calo)
    public static void getIngredientsByDishId(int dishId, DishIngredientsCallback callback) {
        new AsyncTask<Void, Void, Object>() {
            @Override
            protected Object doInBackground(Void... voids) {
                try {
                    Connection connection = DatabaseConnection.getConnection();
                    if (connection != null) {
                        String query = "SELECT id, dish_id, ingredient_name, quantity, calories_per_unit " +
                    ...
                    Log.e("DishDAO", "Error in getIngredientsByDishId: " + e.getMessage());
                    return e.getMessage();
                }
            }
            ...
            protected void onPostExecute(Object result) {
                if (result instanceof List) {
                    ...
                    callback.onSuccess((List<DishIngredient>) result);
                } else {
                    callback.onError(result.toString());
                }
            }
